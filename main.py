...
def ask_folder_path(default="."):
    ...
    if not __import__("os").path.isdir(path):
        print(f"❌ Folder '{path}' tidak ditemukan."); return None
    return path

def run_agent(choice):
    try:
        if choice == "1":
            from agents.basic import main as agent_main; agent_main()
        elif choice == "2":
            from agents.memory import main as agent_main; agent_main()
        elif choice == "3":
            folder = ask_folder_path()
            if folder:
                from agents.rag import main as agent_main; agent_main(folder_path=folder)
        elif choice == "4":
            folder = ask_folder_path()
            if folder:
                from agents.editor import main as agent_main; agent_main(folder_path=folder)
        elif choice == "5":
            from agents.cloud import main as agent_main; agent_main()
        else:
